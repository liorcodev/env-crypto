 ...
 * @throws Error if the key is missing or file operations fail.
 */
export function encryptEnvFile(
  ...
  // Write encrypted file with salt, IV and auth tag
 ...
 * @throws Error if the key is missing, file is invalid, or decryption fails.
 */
export function decryptEnvFile(
  ...
  const key = process.env[keyEnvVar];
  if (!key) {
    throw new Error(`Decryption key not found in environment variable ${keyEnvVar}`);
  }
